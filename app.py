...
from ewt.ewt2d import ewt2dTensor
...
def apply_ewt_preprocessing(image):
    """
    Apply Empirical Wavelet Transform (EWT) preprocessing to an RGB image using 2D EWT (Tensor).
    Returns the reconstructed image from the first EWT subband for visualization.
    """
    # Convert to grayscale for EWT
    gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
    gray = gray.astype(np.float64)
    # Set EWT parameters (can be tuned)
    params = ewt_params(N=3, detect='scalespace', log=0, real=1)
    # Apply 2D EWT (Tensor)
    ewtc, mfb_row, mfb_col, bounds_row, bounds_col = ewt2dTensor(gray, params)
    # Use the first subband for enhancement (as in original intent)
    reconstructed = ewtc[0][0]
    # Normalize to 0-255 and convert to uint8
    reconstructed = np.clip(reconstructed, 0, 255)
    reconstructed = reconstructed.astype(np.uint8)
    # Convert back to RGB for display
    enhanced_rgb = cv2.cvtColor(reconstructed, cv2.COLOR_GRAY2RGB)
    return enhanced_rgb
...
def process_image(image_id):
    ...
    try:
        filepath = os.path.join(app.config['UPLOAD_FOLDER'], image_record.image_path)
    ...
    # Save EWT processed image as grayscale PNG
    ewt_processed_filename = f"ewt_processed_{image_id}_{uuid.uuid4()}.png"
    ewt_processed_path = os.path.join('static/heatmaps', ewt_processed_filename)
    plt.imsave(ewt_processed_path, cv2.cvtColor(processed_image, cv2.COLOR_RGB2GRAY), cmap='gray')
    ewt_processed_relative_path = f"heatmaps/{ewt_processed_filename}"
    ...
    return render_template('result.html', 
                         ...
                         ewt_processed_path=ewt_processed_relative_path,
                         classes=classes,
                         category=category,
